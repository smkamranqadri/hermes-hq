  ...
  return (
    <section className="mx-auto max-w-6xl p-4 sm:p-6">
      ...
      <GlassCard className="mb-5 mt-2 p-0">
        {[['Projects', '/projects'] as const, ['Agents', '/agents'] as const, ...TOOLS, ['System', '/system'] as const].map(([label, to]) => <Link key={to} to={to} className={row}><span>{label}</span><span className="text-muted">›</span></Link>)}
      </GlassCard>
