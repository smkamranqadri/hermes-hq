...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <Ctx.Provider value={push}>
      {children}
      <div className="pointer-events-none fixed inset-x-0 bottom-[calc(env(safe-area-inset-bottom,0px)+4.5rem)] z-50 sm:bottom-4 flex flex-col items-center gap-2 px-4">
        {list.map(t => (
