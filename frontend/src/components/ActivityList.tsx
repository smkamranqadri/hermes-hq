...
export function ActivityList({ events, compact = false }: { events: ActivityEvent[]; compact?: boolean }) {
  ...
  return (
    <div className="glass min-w-0 divide-y divide-line-subtle overflow-hidden rounded-xl">
      {events.map(e => (
        <div key={`${e.kind}-${e.id}`} className="flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-0.5 px-3 py-2 text-xs sm:flex-nowrap">
