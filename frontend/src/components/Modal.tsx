...
export const TextArea = (p: React.TextareaHTMLAttributes<HTMLTextAreaElement>) => <textarea {...p} className="w-full rounded-lg border border-line bg-inset px-3 py-2 text-sm outline-none focus:border-accent" rows={p.rows ?? 3} />
...
export function Btn({ kind = 'primary', ...p }: React.ButtonHTMLAttributes<HTMLButtonElement> & { kind?: 'primary' | 'ghost' | 'warn' }) {
  ...
  return <button {...p} className={`rounded-full px-4 py-1.5 font-mono text-[11px] uppercase tracking-wider disabled:opacity-50 ${cls} ${p.className ?? ''}`} />
}
