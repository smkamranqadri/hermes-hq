...
export function Bell() {
  ...
  // phones: drop under the bell like the other menus, but span the screen width (fixed, so it never overflows)
  useEffect(() => { if (!open) return; const r = ref.current?.getBoundingClientRect(); setTop(r ? Math.round(r.bottom + 8) : null) }, [open])
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-96 rounded-xl border border-line hq-menu p-2 text-xs shadow-lg max-sm:fixed max-sm:inset-x-2 max-sm:mt-0 max-sm:w-auto" style={window.innerWidth < 640 && top != null ? { top } : undefined} data-bell-menu>
