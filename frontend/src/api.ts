...
export const useModels = (q: string, provider = '', profile = '') => useQuery({ queryKey: ['chat-models', q, provider, profile], queryFn: () => get<{ models: string[]; providers: { id: string; name: string; active: boolean }[]; efforts: string[] }>(`/api/chat/models?q=${encodeURIComponent(q)}&provider=${encodeURIComponent(provider)}&profile=${encodeURIComponent(profile)}`), staleTime: 600000 })
