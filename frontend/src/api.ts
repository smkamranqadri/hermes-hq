...
export type AgentModel = { provider: string; model: string; base_url: string; effort: string }
export const useAgentModel = (name: string, enabled = true) => useQuery({ queryKey: ['agent-model', name], queryFn: () => get<AgentModel>(`/api/agent/${name}/model`), enabled })
