...
Endpoints served here (all authenticated, JSON):
  ...
  files          /api/project/{slug}/files, /api/file?path=  (RO preview)
...
# ---------------------------------------------------------------------------
# files-in-project (native tree + artifacts + RO preview)
# ---------------------------------------------------------------------------

# Directories filtered out of the native tree (size/noise).
EXCLUDE_DIRS = {".git", "node_modules", "__pycache__", ".venv", ".tox",
                ".idea", ".vscode", "dist", "build", "target"}

# F-13: hard bounds for the native tree walk so a large monorepo cannot DoS a
# handler thread or emit a multi-hundred-megabyte JSON body. When a bound is hit
# the walk stops and project_files reports `tree_truncated` honestly (the UI can
# surface it — never a fabricated "complete" tree).
TREE_MAX_ENTRIES = 5000
TREE_MAX_DEPTH = 12


def project_files(db_path, slug):
    """{tree, artifacts} for a project. tree = native walk of primary_path;
    artifacts = every result_path its runs/tasks produced (annotated)."""
    con = connect_ro(db_path)
    try:
        p = _fetchone(con, "SELECT * FROM projects WHERE slug=?", (slug,))
        if p is None:
            return None
        pid = p["id"]
        primary = p["primary_path"] or ""
        tree, tree_truncated = _walk_tree(primary)

        artifacts = []
        seen = set()
        rows = _fetchall(con,
            "SELECT t.id AS task_id, t.title AS task_title, "
            "t.result_paths AS task_paths, r.id AS run_id, "
            "r.agent_profile AS agent, r.session_id AS session_id, "
            "r.result_paths AS run_paths "
            "FROM runs r LEFT JOIN tasks t ON t.id = r.task_id "
            "WHERE t.project_id=? ORDER BY r.id ASC", (pid,))
        for r in rows:
            for path in (_parse_paths(r.get("run_paths"))
                         + _parse_paths(r.get("task_paths"))):
                ...
                if st is None:
                    continue
                ans = {"path": path, "name": os.path.basename(path.rstrip("/")),
                       "size": st["size"], "is_dir": st["is_dir"]}
                ans["task_id"] = r["task_id"]
                ans["task_title"] = r["task_title"]
                ans["agent"] = r["agent"]
                ans["run_id"] = r["run_id"]
                ans["session_id"] = r["session_id"]
                ans["inside_primary"] = _under(primary, path)
                # group: dirs under primary show as a tree entry, else artifact
                artifacts.append(ans)
        return {"primary_path": primary, "tree": tree, "artifacts": artifacts,
...
def _stat(path):
    try:
        st = os.stat(path)
        return {"size": st.st_size if not os.path.isdir(path) else None,
...
def _walk_tree(root):
    """Native tree walk of primary_path (files only). RO; depth + count bounded
    (F-13). Returns (entries, truncated) — `truncated` is True when the walk had
    to stop early because it hit TREE_MAX_ENTRIES or TREE_MAX_DEPTH, so callers
    can surface the limit honestly instead of presenting a partial tree as full."""
    out = []
    truncated = False
    ...
    try:
        ...
        for base, dirs, files in os.walk(rroot):
            ...
            rel = os.path.relpath(base, rroot)
            depth = 0 if rel == "." else rel.count(os.sep) + 1
            if depth >= TREE_MAX_DEPTH:
                if dirs:
                    truncated = True  # subdirectories this deep are pruned
                dirs[:] = []
            for fn in files:
                ...
                rp = os.path.relpath(full, rroot)
                st = _stat(full)
                out.append({
                    "path": rp if rp != "." else fn,
                    "name": fn,
...
TEXT_SUFFIXES = {".txt", ".md", ".mdx", ".json", ".yaml", ".yml", ".toml",
                 ".py", ".js", ".ts", ".jsx", ".tsx", ".css", ".html", ".htm",
                 ".sh", ".bash", ".md", ".rst", ".csv", ".ini", ".cfg",
                 ".env.example", ".sql", ".svg", ".xml", ".log"}


def file_preview(db_path, slug, rel_path, max_chars=30000):
    """RO file preview: only text-ish files under the project's primary_path.
    Returns None (404) if not found/inside; raises ValueError on protected."""
    con = connect_ro(db_path)
    try:
        p = _fetchone(con, "SELECT * FROM projects WHERE slug=?", (slug,))
        if p is None:
        ...
        primary = p["primary_path"] or ""
        if not primary:
            raise ValueError("project has no primary_path")
        rp = os.path.realpath(primary)
        target = os.path.realpath(os.path.join(primary, rel_path.lstrip("/")))
        if not (target == rp or target.startswith(rp + os.sep)):
            raise ValueError("path escapes primary_path")
        if not os.path.isfile(target):
            return None
        ext = os.path.splitext(target)[1].lower()
        stat = os.stat(target)
        if ext not in TEXT_SUFFIXES or stat.st_size > (max_chars + 1000):
            return {"path": rel_path, "name": os.path.basename(target),
                    "size": stat.st_size, "text": None,
                    "note": "binary / non-text / too large — preview not shown"}
        with open(target, "r", encoding="utf-8", errors="replace") as f:
            text = f.read(max_chars)
        if len(text) >= max_chars:
            text += "…[truncated]"
        return {"path": rel_path, "name": os.path.basename(target),
                "size": stat.st_size, "text": text, "note": None}
    finally:
        con.close()
