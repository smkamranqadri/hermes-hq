...
- hermes-hq runs **inside** the Hermes s6 container: `/run/service/gateway-<profile>` slots exist for every profile, `hermes --profile X gateway start|stop` = `s6-svc -u|-d`. Specialist gateways stay down until `.env` has `API_SERVER_PORT`/`API_SERVER_KEY`; the default gateway (`:8642`, key in `/opt/data/.env`) is container-owned. `hermes profile create` also registers an s6 slot — deleting the directory alone leaves the slot behind (clean with `hermes_cli.profiles._maybe_unregister_gateway_service`).
