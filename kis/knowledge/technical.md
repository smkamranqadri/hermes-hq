...
- **Theming**: all colors are `--hq-*` CSS vars per `[data-theme]` in `web/src/index.css`, exposed to Tailwind via `@theme inline`. Themes: violet (default, from WM tokens), nous (teal/cream/amber), nous-light (the only light theme; off-white/navy/blue from workspace `claude-nous-light`), bronze (charcoal/bronze), slate, hermes (indigo) — palettes taken from hermes-workspace `styles.css`. No Auto/OS-follow mode (owner removed it 2026-08-29). Pref stored in `localStorage['hq-theme']`, `?theme=<id>` overrides (used for screenshots). **Fonts**: only Inter + JetBrains Mono are bundled (`web/public/fonts`); one font choice in the Appearance menu: JetBrains Mono (default, everywhere), Inter, System Sans/Serif/Mono (device fonts). Stored in `localStorage['hq-fonts']`, `?font=<id>` overrides. Decision 2026-08-29: no extra font packages — the app renders on each viewer's device, so bundling is the only way to look identical, and owner only wants JetBrains. Navbar anatomy copied from WM v0.9: full-width glass bar, icon tile + `HERMES // HQ` wordmark + version pill, bordered pill-group nav, sysbar (TOOLS, appearance, pulsing LIVE dot, clock, SYSTEM pill). Style rules: glass cards (`glass`/`glass-strong` utilities), orb + dot-grid body background, Inter + JetBrains Mono bundled in `web/public/fonts`.
