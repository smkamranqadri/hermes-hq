...
**2b — Review polish + Library filters.** wiki-update (diff preview) + contradiction (`disputed`, keep-both) + new-task proposal kinds, per-item edit/defer + human-feedback field the librarian reads on revision, lint lane (deterministic checks → notification), the deferred Library filters: project/tag within an area (list_notes tag param), and the enforced closed tag taxonomy (2a shipped discipline only).
**2b interview candidates (owner-decided, raised 2026-09-06 from live 2a use):** (a) **archive/junk proposal kind** — the librarian had no way to say "this capture is junk, archive it" and filed 'asdfasd' to Journal as the least-bad option; (b) **capture-triggered ingest nudge** — owner questioned the 30-min scheduler latency; a capture could arm the next tick immediately, or a "Triage now" button on Brain Home; (c) **team-roster refresh** — the six older specialists' SOUL/MEMORY rosters predate Librarian and don't list it.
