...
Ledger: [x] stop-run (done 2026-08-29, see Proof) · [x] templates extract (done 2026-08-29) · [x] agents API (done 2026-08-29) · [x] gateway supervisor (done 2026-08-29, s6-driven) · [ ] Agents UI.
...
## Proof (latest)
- Gateways 2026-08-29: `pytest tests/backend` 30 passed (3 new in `test_gateways.py` with a fake `hermes` whose `gateway start` serves `/v1/models`: enable appends PORT/KEY marked `# hermes-hq` leaving owner lines intact, idempotent, disable stops, activity `gateway_start/stop`; idle sweep after 15 min; `ensure_running`; `stop_started` on exit; refusals for default/unknown/not-installed/not-enabled). Real on this box: `set_enabled("coder", True)` → `.env` +2 lines, s6 `gateway-coder` PID up in 3.8s, `:8653` LISTEN, `/v1/models` with the key returns model `coder`; `set_enabled(False)` → port closed, no gateway process, owner `.env` lines byte-identical. Stray `gateway-probe` s6 slot from the earlier scratch profile unregistered via Hermes' own `_maybe_unregister_gateway_service`.
